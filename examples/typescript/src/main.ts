...
async function main() {
  ...
  try {
    const playerResponse = await client.getPlayer(
      "ac29411d0826412f98c0dd14b334c1fa",
    );
    ...
    const productCount = bazaar.products
      ? Object.keys(bazaar.products).length
      : 0;
    ...
    if (bazaar.products) {
      Object.keys(bazaar.products)
        ...
        .forEach((productId) => {
          ...
          console.log(
            `- ${productId}: Buy Price ${product?.quick_status?.buyPrice}`,
          );
        });
    }
  } catch (error) {
